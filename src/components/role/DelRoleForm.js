...
const DelRoleForm = () => {
  ...
  const { handleCloseDel, handleDelRole, delRoleObj } = useContext(RoleContext);
  React.useEffect(() => {
    if (status === 'completed') {
      if (data) {
        handleCloseDel();
        swal('Xóa thành công!', 'Bạn đã xóa chức vụ thành công', 'success');
        handleDelRole(delRoleObj);
      } else if (error) swal('Đã có lỗi xảy ra', error, 'error');
    }
  }, [data, error, status, handleCloseDel, handleDelRole, delRoleObj]);
